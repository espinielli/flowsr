...
We need a file that specifies which countries to show, something like:
...
Practically we use a threshold to filter out the countries with small flows because
they will result in very thin strips that will just clutter the interpretation
of the flows.

Say we only keep the flows with at least `threshold` flights (in any direction)
...
* `TODO`: generate country file for intra-flow too. 
